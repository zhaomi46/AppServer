...
public interface ErrandMapper {
    @Insert("INSERT INTO errand (description,pic, taskId)" +
            " Values(#{description},#{pic}, #{taskId})")
    @Options(useGeneratedKeys = true, keyProperty = "errandId")
    int insertErrand(Errand errand);

    @Update("UPDATE errand set description=#{description},pic=#{pic}, taskId=#{taskId} where errandId=#{errandId}")
